...
REQUIRED_DOCS = (
    "docs/README.md", "docs/ARCHITECTURE.md", "docs/APP_ARCHITECTURE.md", "docs/BOOTLOADER.md",
    "docs/FLASH_LAYOUT.md", "docs/PROTOCOL.md", "docs/PROTECTION.md", "docs/STATE_MACHINE.md",
    "docs/PARAMETERS.md", "docs/NVM.md", "docs/EVENT_LOG.md", "docs/SOC.md", "docs/BLE.md",
    "docs/UPPER_COMPUTER.md", "docs/CONFIGURATION.md", "docs/CLOUD_CI.md", "docs/HIL.md", "docs/RELEASE.md",
    "docs/engineering/CODING_STANDARD.md", "docs/engineering/TESTING.md", "docs/engineering/AI_DEVELOPMENT.md",
    "docs/engineering/DOCUMENTATION_POLICY.md", "docs/engineering/TRACEABILITY.md", "docs/adr/README.md",
)
RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("bootloader/", ("docs/BOOTLOADER.md",)), ("protocol/", ("docs/PROTOCOL.md",)),
    ("app/core/src/bms_protection", ("docs/PROTECTION.md",)), ("app/core/include/bms_protection", ("docs/PROTECTION.md",)),
    ("app/core/src/bms_mos", ("docs/PROTECTION.md",)), ("app/core/include/bms_mos", ("docs/PROTECTION.md",)),
...
SPECIAL_RULES = (
    ...
    (("flash",), ("docs/FLASH_LAYOUT.md",)), (("ble",), ("docs/BLE.md",)), (("release",), ("docs/RELEASE.md",)),
)

def git_changed(base: str) -> set[str]:
    result = subprocess.run(["git", "diff", "--name-only", f"{base}...HEAD"], cwd=ROOT, check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def structure_gate() -> None:
    ...
    for rel in REQUIRED_DOCS:
        path = ROOT / rel
        if not path.is_file(): failures.append(f"missing required document: {rel}")
        elif path.stat().st_size < 250: failures.append(f"owner document too small to be useful: {rel}")
...
def change_coupling_gate(base: str) -> None:
    ...
    for changed_path in sorted(changed):
        ...
        for prefix, docs in RULES:
            if changed_path.startswith(prefix): required.update(docs)
        for keywords, docs in SPECIAL_RULES:
            if all(keyword in lower for keyword in keywords): required.update(docs)
        for owner in sorted(required):
            if owner not in changed_docs: failures.append(f"{changed_path} changed without synchronized owner document {owner}")
    if failures:
...
def main() -> int:
    ...
    if base: change_coupling_gate(base)
    else: print("DOCUMENTATION_CHANGE_COUPLING_SKIP (BMS_DOCS_BASE_SHA not set)")
    return 0
