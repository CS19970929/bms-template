...
def sync(repo: str, commit: str, dest: Path, files: dict[str, tuple[str, str]]) -> int:
    ...
    for rel, (source, expected) in files.items():
        ...
        if actual != expected:
            print(
                f"hash mismatch {source}: expected {expected}, got {actual}",
                file=sys.stderr,
            )
            return 2
...
F1 = {
    "CMSIS/core_cm3.c": (F1_BASE + "drivers/core_cm3.c", "fcff0d133ca83a837ea4a2076d4fc629e14d75b9"),
    "CMSIS/core_cm3.h": (F1_BASE + "drivers/core_cm3.h", "7ab7b4b43685d3ab2facb53d326c48eeb2bdfda1"),
    "CMSIS/stm32f10x.h": (F1_BASE + "drivers/stm32f10x.h", "18ea2a7e47f5cb3251cee0b277eadc39ce5bb07e"),
    "CMSIS/system_stm32f10x.h": (F1_BASE + "drivers/system_stm32f10x.h", "54bc1abe722b2a82fe0fd9fb1b7acb937597a2c2"),
    "StdPeriph/inc/misc.h": (F1_BASE + "inc/misc.h", "9a6bd07c17cbe6a24cd23b31c99554f58bcfa5dc"),
    "StdPeriph/inc/stm32f10x_flash.h": (F1_BASE + "inc/stm32f10x_flash.h", "63720dea3446d5e5ef9b078866c79a5e80d99352"),
    "StdPeriph/inc/stm32f10x_gpio.h": (F1_BASE + "inc/stm32f10x_gpio.h", "dd28da89c0e1e03c843c2dd73a4c5816c280dd72"),
    "StdPeriph/inc/stm32f10x_iwdg.h": (F1_BASE + "inc/stm32f10x_iwdg.h", "25b0bb5206b3ed65327c5df08413a895e620b0ea"),
    "StdPeriph/inc/stm32f10x_rcc.h": (F1_BASE + "inc/stm32f10x_rcc.h", "1149c34777ac24b415bf9cefbf80d11d9f0a7a7a"),
    "StdPeriph/inc/stm32f10x_usart.h": (F1_BASE + "inc/stm32f10x_usart.h", "162fa87ccebd3d9261a5841c982712294c9e4908"),
    "StdPeriph/src/misc.c": (F1_BASE + "src/misc.c", "c0a5e11333d7043a2cb490975a9301c27ecf524a"),
    "StdPeriph/src/stm32f10x_flash.c": (F1_BASE + "src/stm32f10x_flash.c", "cdff9e9b84614df645f1ba683bd3effea83448da"),
    "StdPeriph/src/stm32f10x_gpio.c": (F1_BASE + "src/stm32f10x_gpio.c", "d392c6434ba23f2c60fb715bb817cc24dbd072df"),
    "StdPeriph/src/stm32f10x_iwdg.c": (F1_BASE + "src/stm32f10x_iwdg.c", "c7cbf7ec5ce4a45558c0f2d87084122b5689e54b"),
    "StdPeriph/src/stm32f10x_rcc.c": (F1_BASE + "src/stm32f10x_rcc.c", "a29034bdf72b6b69e204c2841f9ce8ba760113bd"),
    "StdPeriph/src/stm32f10x_usart.c": (F1_BASE + "src/stm32f10x_usart.c", "8bddd0b7afd88bf00a4156ba9c4fc17cabc41baf"),
}

F0_REFERENCE_COMMIT = "3cc2db3f263bb466ed2292dfbc75d4a7f09c223c"
F1_REFERENCE_COMMIT = "8420ceaa983341e962f62e54eb6f8a627917cc07"


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--family", choices=["f0", "f1", "all"], default="all")
    args = parser.parse_args()

    if args.family in ("f0", "all"):
        rc = sync(
            "CS19970929/030-309-template",
            F0_REFERENCE_COMMIT,
            ROOT / "vendor/st/stm32f0_stdperiph_v1.5.0",
            F0,
        )
        if rc:
            return rc
    ...
    if args.family in ("f1", "all"):
        rc = sync(
            "CS19970929/103-309-template",
            F1_REFERENCE_COMMIT,
            ROOT / "vendor/st/stm32f10x_stdperiph_v3.5.0",
            F1,
        )
        if rc:
            return rc
