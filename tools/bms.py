...
def print_targets() -> None:
    for name in target_names():
        ...
        print(f"{name}: mcu={cfg['mcu']['part']} afe={cfg['afe']['name']} board={cfg['board']['name']} product={cfg['product']['name']} cells={cfg['product']['cell_count']} app={cfg['flash']['app_size']}B")
...
def build_firmware(target: str) -> None:
    ...
    bootstrap = {"stm32f0": "f0", "stm32f1": "f1"}.get(cfg["mcu"]["family"])
    if bootstrap is None:
        raise SystemExit(f"no vendor bootstrap mapping for MCU family {cfg['mcu']['family']}")
    run([sys.executable, "tools/bootstrap_vendor.py", "--family", bootstrap])
    build_dir = ROOT / "build/targets" / target
    run(["cmake", "--fresh", "-S", str(ROOT), "-B", str(build_dir), "-G", "Ninja", f"-DCMAKE_TOOLCHAIN_FILE={ROOT / 'cmake/toolchains/arm-none-eabi.cmake'}", "-DCMAKE_BUILD_TYPE=Release", "-DCMAKE_C_FLAGS_RELEASE=-O2 -DNDEBUG", "-DBMS_BUILD_HOST_TESTS=OFF", f"-DBMS_TARGET={target}"])
    run(["cmake", "--build", str(build_dir)])
    run([sys.executable, "tools/check_firmware_image.py", "--build-dir", str(build_dir), "--target", target])


def build_pc() -> None:
    require("dotnet")
    run(["dotnet", "restore", "pc/BmsTool.sln", "--nologo"])
    run(["dotnet", "build", "pc/BmsTool.sln", "-c", "Release", "--no-restore", "--nologo", "-warnaserror"])
    run(["dotnet", "run", "--project", "pc/tests/Bms.Protocol.Smoke/Bms.Protocol.Smoke.csproj", "-c", "Release", "--no-build"])
...
def main() -> int:
    ...
    if args.command == "targets":
        ...
        run([sys.executable, "tools/generate_events.py", "--write"])
